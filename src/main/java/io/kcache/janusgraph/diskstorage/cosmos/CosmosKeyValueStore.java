...
public class CosmosKeyValueStore implements OrderedKeyValueStore {

    private static final Logger log = LoggerFactory.getLogger(CosmosKeyValueStore.class);
    ...
    public CosmosKeyValueStore(String name, CosmosStoreManager manager) {
        this.name = name;
    ...
    private Map<?, ?> getConfigs(String topic) {
        ...
        if (config.has(CosmosConfigOptions.BOOTSTRAP_SERVERS)) {
            String bootstrapServers = config.get(CosmosConfigOptions.BOOTSTRAP_SERVERS);
