...
import static io.kcache.janusgraph.diskstorage.cosmos.CosmosConfigOptions.TOPIC_PREFIX;
import static org.janusgraph.graphdb.configuration.GraphDatabaseConfiguration.GRAPH_NAME;

public class CosmosStoreManager extends AbstractStoreManager implements OrderedKeyValueStoreManager {

    private static final Logger log = LoggerFactory.getLogger(CosmosStoreManager.class);

    private final Map<String, CosmosKeyValueStore> stores;

    protected final StoreFeatures features;

    public CosmosStoreManager(Configuration configuration) throws BackendException {
        super(configuration);
    ...
    public StoreTransaction beginTransaction(final BaseTransactionConfig txCfg) throws BackendException {
        return new CosmosTx(txCfg);
    }

    @Override
    public CosmosKeyValueStore openDatabase(String name) throws BackendException {
        Preconditions.checkNotNull(name);
        if (stores.containsKey(name)) {
            return stores.get(name);
        }
        try {
            CosmosKeyValueStore store = new CosmosKeyValueStore(name, this);
    ...
    public void mutateMany(Map<String, KVMutation> mutations, StoreTransaction txh) throws BackendException {
        for (Map.Entry<String,KVMutation> mutation : mutations.entrySet()) {
            CosmosKeyValueStore store = openDatabase(mutation.getKey());
    ...
    public void close() throws BackendException {
        for (CosmosKeyValueStore store : stores.values()) {
            store.shutDown();
        }
        stores.clear();
    }
